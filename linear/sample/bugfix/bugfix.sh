...
run_script() {
  ...
  # You should probabably modify this to use your preferred terminal CLI instead
  if ! claude --dangerously-skip-permissions "$REMOTE_PROMPT"; then
    status=$?
    printf 'Failed to launch Claude remote session (exit %s)\n' "$status" >&2
    exit "$status"
  fi
